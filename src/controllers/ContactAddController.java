...
public class ContactAddController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        firstName.setOnKeyPressed(event->{if(event.getCode()==KeyCode.ENTER){inputMethod();}});
        lastName.setOnKeyPressed(event->{if(event.getCode()==KeyCode.ENTER){inputMethod();}});
        phone.setOnKeyPressed(event->{if(event.getCode()==KeyCode.ENTER){inputMethod();}});
        email.setOnKeyPressed(event->{if(event.getCode()==KeyCode.ENTER){inputMethod();}});
        adress.setOnKeyPressed(event->{if(event.getCode()==KeyCode.ENTER){inputMethod();}});
        message.setOnKeyPressed(event->{if(event.getCode()==KeyCode.ENTER){inputMethod();}});
        clear();
    }    
    ...
    private void inputMethod(){
        if(firstName.getText().equals("")||lastName.getText().equals("")||adress.getText().equals("")
            ...
            if(model.getStatusInert()==true){
                nav.showAlert(Alert.AlertType.INFORMATION, "Success", null, "The Contact has been saved..");
                clear();
            }
            else{
